...
void Gearreport503503::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_mjev()->mutable_gearreport503_503()->set_gearstatefault(gearstatefault(bytes, length));
  chassis->mutable_mjev()->mutable_gearreport503_503()->set_gear_stateact(gear_stateact(bytes, length));
}
...
Gearreport503_503::GearstatefaultType Gearreport503503::gearstatefault(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
  int32_t x = t0.get_byte(0, 1);

  Gearreport503_503::GearstatefaultType ret =  static_cast<Gearreport503_503::GearstatefaultType>(x);
  return ret;
}
...
Gearreport503_503::Gear_stateactType Gearreport503503::gear_stateact(const std::uint8_t* bytes, int32_t length) const {
  Byte t0(bytes + 0);
  int32_t x = t0.get_byte(1, 3);

  Gearreport503_503::Gear_stateactType ret =  static_cast<Gearreport503_503::Gear_stateactType>(x);
  return ret;
}
}  // namespace mjev
}  // namespace canbus
