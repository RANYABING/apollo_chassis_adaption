...
void Throttlereport500500::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_MJCar()->mutable_throttlereport500_500()->set_throttlepedalact(throttlepedalact(bytes, length));
  chassis->mutable_MJCar()->mutable_throttlereport500_500()->set_drivecomfault(drivecomfault(bytes, length));
  chassis->mutable_MJCar()->mutable_throttlereport500_500()->set_drivehwfault(drivehwfault(bytes, length));
  chassis->mutable_MJCar()->mutable_throttlereport500_500()->set_throttleenstate(throttleenstate(bytes, length));
}
