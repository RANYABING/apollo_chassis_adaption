...
void Brakereport501501::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_MJCar()->mutable_brakereport501_501()->set_brakepedalact(brakepedalact(bytes, length));
  chassis->mutable_MJCar()->mutable_brakereport501_501()->set_brakehwfault(brakehwfault(bytes, length));
  chassis->mutable_MJCar()->mutable_brakereport501_501()->set_brakecomfault(brakecomfault(bytes, length));
  chassis->mutable_MJCar()->mutable_brakereport501_501()->set_brakeenstate(brakeenstate(bytes, length));
}
